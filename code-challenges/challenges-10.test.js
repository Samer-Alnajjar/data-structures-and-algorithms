...
const count = (target, input) => {
  // Solution code here...
  let oneDimArray = input.reduce((accumulator,index)=>accumulator.concat(index),[]);
  let count=0;
  oneDimArray.map(element=>{
    if (element==target){
      count+=1;
    }
  })
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(innerArray => {
    newArr.push(innerArray.filter(value => typeof value === 'number' && value % 5 === 0).map(value => Math.pow(2, value)))
  })
  return newArr;
};
